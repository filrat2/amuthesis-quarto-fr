...
indices}), które były wykorzystywane w poprzednich badaniach przez
\textcite{zhang_2021_texture}, \textcite{plakman_2022_pv},
...
\item
  ...
  , \(SWIR1\) -- reflektancja w kanale średniej podczerwieni, \(NIR\) --
  reflektancja w kanale bliskiej podczerwieni
\item
...
przestrzenne) \autocite{grass_r_texture}. Model tekstury oparty na
macierzy współwystępowania poziomów szarości (ang. \emph{Gray Level
Co-Occurrence Matrix}, GLCM), zaproponowany przez
\textcite{haralick_1973_texture}, jest często używany do obliczania
tekstur obrazu. Ta metoda polega na tworzeniu macierzy opisującej
częstotliwość występowania par wartości w określonym fragmencie obrazu,
uwzględniając określone sąsiedztwo, kierunki i przerwy między pikselami
\autocite{kupidura_2019_texture}.
...
przeprowadził \autocite{zhang_2021_texture} dotyczące wykorzystania
...
pracy skorzystano jedynie z tekstury średniej sumy (ang. \emph{Sum
Average}, SA lub SAVG), wskazanej przez \textcite{wang_2022_pv} jako
...
\emph{resample} z pakietu \emph{terra} \autocite{R-terra}, wykorzystując
...
oraz siatki dane zostały następnie użyte do obliczeń filtracji
teksturalnych oraz wskaźników teledetekcyjnych. Po uzyskaniu produktów
pochodnych, w zależności od wariantu, dane zostały scalone w kilka
wielokanałowych rastrów, które posłużyły do wyodrębnienia zestawu danych
treningowych oraz do przeprowadzenia predykcji.
