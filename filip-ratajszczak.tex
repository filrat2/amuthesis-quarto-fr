...
losowych dla poszczególnych wariantów (zbiorów danych) zostały
przedstawione w tabeli \ref{tbl-tabela-tuning}. Warto zauważyć, że
warianty 3 i 6 wykazują identyczne wartości dla parametrów
\texttt{sample.fraction}, \texttt{min.node.size}, \texttt{num.trees} i
\texttt{max.depth}. Podczas losowania hiperparametrów dla każdego
wariantu wykorzystano to samo ziarno losowości (ang. \emph{random
seed}), ustawione za pomocą funkcji \texttt{set.seed()}. Dla dwóch
wspomnianych wariantów najlepsze wyniki zostały osiągnięte przy
wykorzystaniu identycznego zestawu hiperparametrów.
...
jakości, przeznaczonych dla klasyfikatorów binarnych:
...
\item
  ...
  negatywna \autocite{R-mlr3measures}. Wartości AUC mieszczą się w
  zakresie od 0 do 1, gdzie wartość 0,5 lub niższa oznacza oznacza model
  nie lepszy od losowego, a 1,0 -- doskonałe przewidywanie obu klas.
...
Jakość modeli za pomocą wymienionych miar została oceniona za pomocą
przestrzennej walidacji krzyżowej, która uwzględnia szczególny charakter
danych geograficznych \autocite{lovelace_2019_geocomputation}.
